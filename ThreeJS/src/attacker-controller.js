    ...
    _OnAnimAction(m) {
      ...
      if (oldTiming < this._params.timing && this._timeElapsed >= this._params.timing) {
        ...
        const _Filter = (c) => {
          ...
          // Don't let the player attack the girl (only apply to player attacks)
          const girl = c.entity.GetComponent('GirlController');
          if (girl && this._parent.GetComponent('BasicCharacterController')) {
            return false;
          }
