...
export const npc_entity = (() => {
  ...
  class NPCController extends entity.Component {
    ...
    _FindTarget(pos) {
      const _IsAliveTarget = (c) => {
        const h = c.entity.GetComponent('HealthComponent');
        if (!h) {
          return false;
        }
        // Target both player and girl (entities with GirlController)
        const isPlayer = c.entity.Name === 'player';
        const isGirl = c.entity.GetComponent('GirlController') !== undefined;
      ...
      const grid = this.GetComponent('SpatialGridController');
      const nearby = grid.FindNearbyEntities(100).filter(c => _IsAliveTarget(c));

      if (nearby.length == 0) {
        return { direction: new THREE.Vector3(0, 0, 0), target: null, distance: Infinity };
      }

      // Calculate weighted target selection based on distance and target type
      let bestTarget = null;
      let bestWeight = -1;

      for (let candidate of nearby) {
        const distance = this._parent._position.distanceTo(candidate.entity._position);
        const isPlayer = candidate.entity.Name === 'player';
        const isGirl = candidate.entity.GetComponent('GirlController') !== undefined;

        // Calculate weight based on distance (closer = higher weight)
        // Max weight at distance 0, decreases as distance increases
        const maxDistance = 100; // Max detection range
        let weight = Math.max(0, (maxDistance - distance) / maxDistance);

        // Apply priority multipliers
        if (isPlayer) {
          weight *= 1.0; // Player gets normal priority
        } else if (isGirl) {
          weight *= 1.5; // Girl gets double priority
        }
      ...
      if (!bestTarget) {
        bestTarget = nearby[0].entity;
      }

      const dir = this._parent._position.clone();
      dir.sub(bestTarget._position);
      const distance = dir.length();
      dir.y = 0.0;
      dir.normalize();

      return { direction: dir, target: bestTarget, distance: distance };
    }
    ...
    _OnAIWalk(timeInSeconds) {
      ...
      const dirToTarget = targetInfo.direction;
      const targetDistance = targetInfo.distance;

      // Check if close enough to attack (attack range)
      const attackRange = 5.0; // Units for attack range
